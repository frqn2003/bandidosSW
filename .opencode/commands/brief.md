---
description: Arma el brief de una pantalla de Centro Académico. Recibe la HU en formato estándar + tu idea inicial, te hace preguntas sobre lo que falta, y genera el .md completo (con datos del esquema y componentes sugeridos). Se usa antes de /disenar.
agent: build
---
...
- **Rol** (administrador, coordinador, docente, alumno, personal administrativo, cajero)
- **Acción** (qué quiere hacer)
- **Beneficio** (para qué)
- **Entidades de negocio implicadas** (alumnos, docentes, turnos, asistencia, cobranzas, reportes, usuarios, etc.)

Guardá el nombre del brief: `docs/briefs/$ARGUMENTS.md` (si `$ARGUMENTS` es `HU-001`, el archivo es `HU-001.md`).
...
**Regla:** preguntá **solo lo que verdaderamente falta** y que no se pueda inferir razonablemente del brief o del dominio. Si un dato es obvio del brief, no preguntes.
...
2. **Sub-agente de componentes** (delegate → `explore`): pasa la lista de piezas que probablemente necesite la pantalla. Usa `mem_search(query: "disenar/componentes", project: "centro-academico")` + grep sobre `src/components/**` para devolver qué componentes existen y con qué props. También `mem_search(query: "hu/HU-XXX components-used", project: "centro-academico")` para HUs previas del mismo módulo.

3. **(En paralelo si posible):** `mem_search(query: "disenar/design-system")` y `mem_search(query: "disenar/reglas")` para tokens y reglas.
