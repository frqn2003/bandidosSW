...
argument-hint: entidad (ej. materia) + operaciones y campos
---
...
> Versión Claude Code de `.opencode/commands/contract.md` (sin cambios de fondo: este comando nunca dependió de Engram).

Entidad / pedido del usuario: `$ARGUMENTS`
...
2. **Revisar los contratos que YA existen** (siempre, en cada uso del comando):
   ...
   - **Reutilizar** tipos, rutas y campos ya acordados (ej. `usuarioId`, `estado`) en vez de inventar nombres nuevos.
   - **Imitar** exactamente el estilo y la estructura de los contratos existentes.

## Paso 1: Entender el contexto

Preguntar al usuario (si no lo especificó):
1. **Entidad**: ¿qué entidad se contrata? (ej: materia, alumno, profesor)
...
Crear `src/contracts/{entidad}.ts` siguiendo EXACTAMENTE la estructura de `docs/api-contracts.md`
y el estilo de los contratos ya existentes (los revisados en el Paso 0):
...
import type { EntidadResponse } from "@/contracts/{entidad}";

const FIXTURE: EntidadResponse[] = [
  {
...
4. **Manejar los errores del contrato** — catch con el código y el union type de errores
